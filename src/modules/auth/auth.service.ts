...
export class AuthService {
  ...
  constructor(
    ...
    private passwordRepo: Repository<ResetPasswordRequestEntity>,
  ...
  generateVerifyToken(validUser: any): string {
    const payload = {
      email: validUser.email,
      name: validUser.firstname_lastname,
    };
  ...
  async createResetPasswordRequest(
    ...
    try {
      ...
      return {
        status: 'success',
        result: data.userIsSignedIn
          ? result.id
          : 'Password reset request created successfully. Please check your email for more info on how to reset the password',
      };
    } catch (error) {
  ...
  async fulfilPasswordRequest(
    id: string,
    updateData: UpdatePasswordRequestDto,
  ): Promise<any> {
    ...
    if (!requestFound) {
      throw new NotFoundException(
        `Link is expired or has already been used to reset pasword`,
      );
    }

    try {
      const updatedUser = this.authRepository.merge(
        requestFound.users,
        updateData,
      );
      await this.authRepository.save(updatedUser);
      //update the active_status of the request so that it cannot be used again
      const updateRequest = this.passwordRepo.merge(requestFound, {
        isActive: false,
      });
  ...
  private async sendResetPasswordEmail(token: string, email: string) {
    ...
    const compose =
      `Hello! <br><br> A request to reset your vibrant creator app account has been initiated.<br><br>` +
