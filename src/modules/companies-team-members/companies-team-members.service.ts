...
export class CompaniesTeamMembersService {
  ...
  async getAll(
    ...
    const totalCount = await this.companyTeamMemberRepo
  ...
  async getInvitationById(params: ValidParamId): Promise<any> {
    ...
    if (result) {
      ...
      throw new NotFoundException(
        `${this.entity_prefix_name} Invitation with ID '${params.id}' not found`,
      );
    }
  }
  ...
  async create(
    params: ValidParamId,
    newData: CreateCompanyTeamMemberDto,
  ): Promise<any> {
    ...
    if (requestFound) {
      ...
      if (alreadyTeamMember) {
        ...
        throw new HttpException(
          'Email already exists as a team member',
  ...
  async update(
    params: ValidParamId,
    user: User,
    updateData: UpdateCompanyTeamMemberDto,
  ): Promise<any> {
    ...
    if (!requestFound) {
      throw new NotFoundException(
        `${this.entity_prefix_name} with ID '${params.id}' by current user cannot be found `,
      );
    }
  ...
  async delete(params: ValidParamId, user: User): Promise<any> {
    ...
    if (!requestFound) {
      throw new NotFoundException(
        `${this.entity_prefix_name} with ID '${params.id}' cannot be found `,
      );
    }
    ...
    if (result.affected === 0) {
      throw new NotFoundException(
        `${this.entity_prefix_name} with ID "${params.id}" could not be deleted`,
      );
    }
  ...
  private async findOneEntityById(params: ValidParamId, user: User) {
    const requestFound = await this.companyTeamMemberRepo
      .createQueryBuilder('company_team_members')
      .where('company_team_members.company = :id', { id: params.companyId })
  ...
  private async sendInvitationEmail(
    ...
    const compose =
      `Hello! <br><br> You have been invited to be part a team member of <b>${company.company_name.toUpperCase()}</b>.<br><br>` +
